...
// Variables can be defined inline as well.
const { data } = await createProject({ name: ..., description: ..., dueDate: ..., status: ..., });
...
// Variables can be defined inline as well.
const ref = createProjectRef({ name: ..., description: ..., dueDate: ..., status: ..., });
...
## AddUserToProject
You can execute the `AddUserToProject` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
addUserToProject(vars: AddUserToProjectVariables): MutationPromise<AddUserToProjectData, AddUserToProjectVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
addUserToProject(dc: DataConnect, vars: AddUserToProjectVariables): MutationPromise<AddUserToProjectData, AddUserToProjectVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the addUserToProjectRef:
```typescript
const name = addUserToProjectRef.operationName;
console.log(name);
```

### Variables
The `AddUserToProject` mutation requires an argument of type `AddUserToProjectVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `AddUserToProject` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `AddUserToProjectData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface AddUserToProjectData {
  projectMembership_insert: ProjectMembership_Key;
}
```
### Using `AddUserToProject`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, addUserToProject, AddUserToProjectVariables } from '@dataconnect/generated';

// The `AddUserToProject` mutation requires an argument of type `AddUserToProjectVariables`:
const addUserToProjectVars: AddUserToProjectVariables = {
  projectId: ..., 
  userId: ..., 
  role: ..., 
};

// Call the `addUserToProject()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await addUserToProject(addUserToProjectVars);
// Variables can be defined inline as well.
const { data } = await addUserToProject({ projectId: ..., userId: ..., role: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await addUserToProject(dataConnect, addUserToProjectVars);

console.log(data.projectMembership_insert);

// Or, you can use the `Promise` API.
addUserToProject(addUserToProjectVars).then((response) => {
  const data = response.data;
  console.log(data.projectMembership_insert);
});
```

### Using `AddUserToProject`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, addUserToProjectRef, AddUserToProjectVariables } from '@dataconnect/generated';
...
// Call the `addUserToProjectRef()` function to get a reference to the mutation.
const ref = addUserToProjectRef(addUserToProjectVars);
// Variables can be defined inline as well.
const ref = addUserToProjectRef({ projectId: ..., userId: ..., role: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = addUserToProjectRef(dataConnect, addUserToProjectVars);
